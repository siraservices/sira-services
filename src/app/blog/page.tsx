        ...
        {posts === undefined ? (
          <div className="space-y-6">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="bg-white rounded-xl border border-surface-border p-6 shadow-soft"
              >
                <div className="animate-pulse">
                  ...
                  <div className="h-6 w-3/4 bg-surface-hover rounded mb-3" />
                  <div className="h-4 w-full bg-surface-hover rounded mb-2" />
                  <div className="h-4 w-2/3 bg-surface-hover rounded mb-4" />
          ...
          <div className="space-y-6">
            {posts.map((post) => (
              <article key={post._id}>
                <Link
                  href={`/blog/${post.slug}`}
                  className="group block bg-white rounded-xl border border-surface-border p-6 shadow-soft hover:shadow-card transition-all duration-200 cursor-pointer"
                >
                  <div className="flex flex-col md:flex-row md:items-center gap-4">
                    <div className="flex-1">
                      ...
                      <h2 className="text-lg font-display font-bold text-text group-hover:text-primary transition-colors duration-200 mb-2">
                        {post.title}
                      </h2>
                    ...
                    <div className="flex items-center gap-4 md:flex-col md:items-end md:gap-3 shrink-0">
                      <time className="text-xs text-text-dim font-display">
                        {post.publishedAt
